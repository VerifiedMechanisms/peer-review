...
// Read-only rendering of one review for the admin: the questionnaire in order, text
// answers as the reviewer saw them in the preview (Markdown, tables, LaTeX). Used by the
// full page (/admin/review/...) and by the bare embed page the admin overlay loads.
export default async function ReviewView({ code, reviewerId, embed }: { code: string; reviewerId: string; embed?: boolean }) {
            ...
            <h1 className="text-2xl font-semibold tracking-tight">
              ...
              {author && (
                <span className="font-normal text-zinc-500">
                  {' '}(
                  {embed ? (
                    <a href={`/api/cv/${code}`} target="_blank" rel="noreferrer" className="hover:underline" title="Open CV">
                      {author}
                    </a>
                  ) : (
                    <Overlay href={`/api/cv/${code}`} title={`${author}, CV`} className="hover:underline">
                      {author}
                    </Overlay>
                  )}
                  )
