...
export const SORT_KEYS = ['code', 'name', 'reviewer', 'status', 'overall', 'quality', 'clarity', 'originality'] as const;
...
// Score columns start highest first, the text columns A-Z.
export const defaultDir = (k: SortKey): SortDir => (SCORE_KEYS.includes(k) || k === 'status' ? 'desc' : 'asc');

// Rows without a value sort last either way; ties fall back to the code.
